...
public class Utils {
     ...
     */
    static String getImageURLFromXkcdApi(String comicNo) throws IOException, JSONException{
    ...
    /**
     * Self-added, for title of comic
     * This method gets the xkcd JSON response from the comic No
     * @param comicNo a String containing the comicNo
     * @return a String containing the JSON response
     */
    static String getComicTitleFromXkcdApi(String comicNo) throws IOException, JSONException{

        final String xckdTitleKey = "safe_title";

        URL url = buildURL(comicNo);
        String jsonString = getJson(url);
        JSONObject jsonObject = new JSONObject(jsonString);
        return jsonObject.getString(xckdTitleKey);
    }
