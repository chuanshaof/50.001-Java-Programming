...
public class AlphabetGenerator {
    ...
    public static char[] generateFrequencyAlphabet(int base, String[] trainingData) {
        ...
        String[] dataClone = trainingData.clone();

        String BASIC_ALPHABET = "abcdefghijklmnopqrstuvwxyz";

        int charCount = 0;
        // Loop through words to gather data
        for (int i = 0; i < dataClone.length; i++) {
            charCount = charCount + dataClone[i].length();
            for (int j = 0; j < dataClone[i].length(); j++) {
                char letter = dataClone[i].charAt(j);
                if (BASIC_ALPHABET.indexOf(letter) != -1) {
                    try{
                        counts.put(letter, counts.get(letter) + 1);
                    } catch (NullPointerException e) {
                        counts.put(letter, 1.0);
                    }
                }
            }
        }
        ...
        // Loop through every element to get CDF
        for (Map.Entry<Character, Double> entry : counts.entrySet()) {
            cdf = cdf + entry.getValue();
            counts.put(entry.getKey(), cdf * base / charCount);
        }
